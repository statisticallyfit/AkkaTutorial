...
object WaysToWireUp extends App {
	...
	source.to(Sink.foreach(println(_))) //is a runnable graph
...
//This is the case when back-pressuring the Publisher is required, because the Subscriber is not able to cope with the rate at which its
// upstream would like to emit data elements.
//
//Since the Publisher is not allowed to signal more elements than the pending demand signalled by the Subscriber, it will have to abide to
...
//As we can see, this scenario effectively means that the Subscriber will pull the elements from the Publisher – this mode of operation is
// referred to as pull-based back-pressure.	.
